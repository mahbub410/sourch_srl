...
select LOCATION_CODE,BOOK,BG,PAGE,CUSTOMER_NAME,ADDRESS,AC_NO,CUSTOMER_NUM,TARIFF,count(1) No_of_month,
sum(lps_amt) lps_amt,sum(Principal_arr) Principal_arr,sum(vat_arr) vat_arr,sum(Total_arr) Total_arr
from (
...
sum(LPS_AMT+LPS_ADJ-LPS_APPL) lps_amt,
sum(PRINCIPAL_AMT+PRINCIPAL_ADJ-PRINCIPAL_APPL) Principal_arr,
sum(VAT_AMT+VAT_ADJ-VAT_APPL) vat_arr,
