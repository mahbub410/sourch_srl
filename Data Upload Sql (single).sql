...
where location_code = 'C7'

select * from  EPAY_UTILITY_BILL_SUP;

TRUNCATE TABLE EPAY_UTILITY_BILL_SUP;
...
WHERE ROWID IN (
SELECT ROWID FROM EPAY_UTILITY_BILL_SUP
MINUS
SELECT MAX(ROWID) FROM EPAY_UTILITY_BILL_SUP
...
DELETE FROM EPAY_UTILITY_BILL_SUP
WHERE ACCOUNT_NUMBER IN (SELECT ACCOUNT_NO FROM EPAY_CUSTOMER_MASTER_DATA);
...
TRUNCATE TABLE EPAY_CUSTOMER_MASTER_DATA_SUP;

    
INSERT INTO EPAY_CUSTOMER_MASTER_DATA_SUP
...
WHERE (CUSTOMER_NUM,LOCATION_CODE) IN (SELECT SUBSTR(ACCOUNT_NUMBER,1,7),LOCATION_CODE FROM EPAY_UTILITY_BILL_SUP);

INSERT INTO EPAY_UTILITY_BILL_SUP select * from EPAY_UTILITY_BILL_GPR@epay_kishor

INSERT INTO EPAY_CUSTOMER_MASTER_DATA_SUP select * from 
EPAY_CUSTOMER_MASTER_DATA_GPR@epay_kishor


COMMIT;


DELETE FROM EPAY_CUSTOMER_MASTER_DATA_SUP
WHERE ROWID IN ( 
SELECT ROWID FROM EPAY_CUSTOMER_MASTER_DATA_SUP
MINUS 
SELECT MAX(ROWID) FROM EPAY_CUSTOMER_MASTER_DATA_SUP
...
SELECT * FROM EPAY_CUSTOMER_MASTER_DATA_SUP
WHERE ACCOUNT_NO IN ( 
SELECT  ACCOUNT_NO FROM EPAY_CUSTOMER_MASTER_DATA_SUP
WHERE LOCATION_CODE='''||LOC||'''
MINUS 
SELECT ACCOUNT_NO FROM CUSTOMER_MASTER_DATA_PDB@EPAY_ROBI
WHERE  LOCATION_CODE='''||LOC||''');'||CHR(10)||CHR(10)||'COMMIT;'||CHR(10) AS "RETURN_QUERY"
FROM (SELECT DISTINCT LOCATION_CODE AS LOC FROM EPAY_CUSTOMER_MASTER_DATA_SUP)
ORDER BY LOC
...
ADVANCE_AMOUNT,ADJ_ADV_GOVT_DUTY FROM EPAY_UTILITY_BILL_SUP
...
SELECT ACCOUNT_NUMBER, BILL_MONTH FROM EPAY_UTILITY_BILL_SUP
...
FROM (SELECT DISTINCT LOCATION_CODE AS LOC,:P_BILL_MONTH BILL_MONTH FROM EPAY_UTILITY_BILL_SUP)
ORDER BY LOC
...
TARIFF,CUSTOMER_TYPE,CURRENT_SURCHARGE,
ARREAR_SURCHARGE,ADJUSTED_PRINCIPLE,ADJUSTED_GOVT_DUTY,
ADVANCE_AMOUNT,ADJ_ADV_GOVT_DUTY,''N'' DATA_TRANS_LOG FROM EPAY_UTILITY_BILL_SUP
...
TARIFF,CUSTOMER_TYPE,CURRENT_SURCHARGE,
ARREAR_SURCHARGE,ADJUSTED_PRINCIPLE,ADJUSTED_GOVT_DUTY,
ADVANCE_AMOUNT,ADJ_ADV_GOVT_DUTY,''T'' DATA_TRANS_LOG FROM EPAY_UTILITY_BILL_SUP
...
WHERE BILL_CYCLE_CODE='''||BILL_MONTH||'''
AND LOCATION_CODE='''||LOC||'''
AND INVOICE_NUM IN (SELECT SUBSTR(BILL_NUMBER,1,LENGTH(BILL_NUMBER)-1) FROM EPAY_UTILITY_BILL_SUP 
                                  WHERE LOCATION_CODE='''||LOC||''');'||CHR(10)||CHR(10)||'COMMIT;'||CHR(10) "QUERY"
FROM (SELECT DISTINCT LOCATION_CODE LOC,:P_BILL_MONTH BILL_MONTH,'@'||'BILLING_MYMEN' DB_LINK  FROM EPAY_UTILITY_BILL_SUP)
ORDER BY LOC
...
WHERE BILL_MONTH='''||BILL_MONTH||'''
AND LOCATION_CODE='''||LOC||''');'||CHR(10)||CHR(10)||'COMMIT;'||CHR(10) AS "RETURN_QUERY"
FROM (SELECT DISTINCT LOCATION_CODE AS LOC, BILL_MONTH FROM EPAY_UTILITY_BILL
