...
--1--BILL CYCLE WISE REP

select * from (
select C.LOCATION_CODE LOCATION_CODE,
M.DESCR,:p20_bill_cycle,
count(c.cust_id) total_customer,
count(g.DATA_UPLOAD_DATE) total_data_upload, 
count(g.DATA_DOWNLOAD_DATE) total_reading_downloaded, count(IMG_UPLOAD_DATE) total_image_uploaded ,count(g.DATA_UPLOAD_DATE)-count(IMG_UPLOAD_DATE) Not_uploaded,
count(h.image_upload_DATE) total_reading,
count(H.DATA_DOWNLOAD_DATE) total_billed
from mbill_customers c,
 mbill_data_log  g ,
MBILL_LOCATION_MASTER M,
mbill_meter_reading_card_hdr h
where c.cust_id=g.cust_id(+)
AND C.LOCATION_CODE=M.LOCATION_CODE
and H.LOCATION_CODE(+)=C.LOCATION_CODE
and H.AREA_CODE(+)=C.AREA_CODE
--AND C.LOCATION_CODE=:P11_LOC
and g.bill_cycle_code(+)=:P20_BILL_CYCLE
and h.bill_cycle_code(+)=:P20_BILL_CYCLE
--and C.LOCATION_CODE in ('M1','M2','M3','I8','I7','V1','V5','V6','V7')
AND ((SELECT ROLE_ID FROM MBILL_USER_MST WHERE USER_NAME=:APP_USER)=1 OR((SELECT ROLE_ID FROM MBILL_USER_MST WHERE USER_NAME=:APP_USER)<>1 AND C.LOCATION_CODE IN(
 SELECT DISTINCT LOCATION_CODE FROM MBILL_METER_READER_ALLOCATION
                        WHERE METER_READER_ID IN(
                         ...
                         AND NVL(EFF_BILL_CYCLE_CODE,:P20_BILL_CYCLE)<=:P20_BILL_CYCLE
                         AND NVL(EXP_BILL_CYCLE_CODE,:P20_BILL_CYCLE)>=:P20_BILL_CYCLE
UNION
SELECT LOCATION_CODE FROM MBILL_USER_LOC_MAP
WHERE USER_ID=(SELECT USER_ID FROM MBILL_USER_MST WHERE USER_NAME=:APP_USER))))
group by C.LOCATION_CODE,M.DESCR,:P20_bill_cycle  --,c.area_code
order by C.LOCATION_CODE )
where (
 instr(upper("LOCATION_CODE"),upper(nvl(:P20_REPORT_SEARCH,"LOCATION_CODE"))) > 0  --or
 --instr(upper("DESCR "),upper(nvl(:P20_REPORT_SEARCH,"DESCR "))) > 0 
)

-----2--LOCATION WISE REP

select SUBSTR(c.area_code,4,2) bill_grp,count(c.cust_id) total_customer,count(g.DATA_UPLOAD_DATE) total_MRS_upload, 
count(g.DATA_DOWNLOAD_DATE) total_reading_dwnld_to_Billing, count(g.IMG_UPLOAD_DATE) total_Reading_Taken ,
count(g.DATA_UPLOAD_DATE)-count(g.IMG_UPLOAD_DATE) Reading_Not_Taken,:P23_BILL_CYCLE Bill_Cycle,:P23_LOCA Location,
sum(decode(rgd_status,null,1,0)) Reading_Unpass
from mbill_customers c,
mbill_data_log g,
MBILL_LOCATION_MASTER M,
mbill_meter_reading_card_hdr h,
mbill_meter_reading_card_dtl cd
where c.cust_id=g.cust_id(+)
and c.cust_id=cd.cust_id(+)
AND C.LOCATION_CODE=M.LOCATION_CODE
AND C.LOCATION_CODE=:P23_LOCA
and g.bill_cycle_code(+)=:P23_BILL_CYCLE
AND h.LOCATION_CODE(+)=:P23_LOCA
and h.bill_cycle_code(+)=:P23_BILL_CYCLE 
and cd.bill_cycle_code(+)=:P23_BILL_CYCLE 
and h.area_code(+)=c.area_code
and h.CARD_GEN_FLAG(+)='R'
AND C.CUSTOMER_STATUS_CODE<>'D'
group by SUBSTR(c.area_code,4,2)
order by substr(c.area_code,4,2)

-----3--BILL GROUP WISW---

select C.LOCATION_CODE LOCATION_CODE,M.DESCR,c.area_code AREA_CODE,count(c.cust_id) total_customer,count(g.DATA_UPLOAD_DATE) total_data_upload, count(g.DATA_DOWNLOAD_DATE) total_reading_downloaded, count(g.IMG_UPLOAD_DATE) total_image_uploaded ,count(g.DATA_UPLOAD_DATE)-count(g.IMG_UPLOAD_DATE) Not_uploaded,count(image_upload_date) Total_Reading
,count(h.data_download_date) Total_Billed_customer
from mbill_customers c,
mbill_data_log g,
MBILL_LOCATION_MASTER M,
mbill_meter_reading_card_hdr h
...
AND C.LOCATION_CODE=M.LOCATION_CODE
AND C.LOCATION_CODE=:P11_LOC
and g.bill_cycle_code(+)=:P11_BILL_CYCLE_CODE
AND h.LOCATION_CODE(+)=:P11_LOC
and h.bill_cycle_code(+)=:P11_BILL_CYCLE_CODE 
and SUBSTR(C.area_code,4,2)=:P11_BILL_GRUOP
and h.area_code(+)=c.area_code
AND ((SELECT ROLE_TYPE FROM MBILL_USER_MST U,MBILL_ROLE_MST R WHERE USER_NAME=:APP_USER AND U.ROLE_ID=R.ROLE_ID)<>3 OR 
 ((SELECT ROLE_TYPE FROM MBILL_USER_MST U,MBILL_ROLE_MST R WHERE USER_NAME=:APP_USER AND U.ROLE_ID=R.ROLE_ID)=3 AND  
   (C.LOCATION_CODE,C.AREA_CODE) IN(SELECT LOCATION_CODE,AREA_CODE FROM MBILL_METER_READER_ALLOCATION
                                    WHERE METER_READER_ID IN(
                                                             SELECT METER_READER_ID FROM MBILL_METER_READER_MST
                                                            where USER_ID<>:APP_USER
                                                            start with USER_ID=:APP_USER
                                                            connect by prior  METER_READER_ID=P_METER_READER_ID)
                                     AND NVL(EFF_BILL_CYCLE_CODE,:P11_BILL_CYCLE_CODE )<=:P11_BILL_CYCLE_CODE 
                                     AND NVL(EXP_BILL_CYCLE_CODE,:P11_BILL_CYCLE_CODE )>=:P11_BILL_CYCLE_CODE)))
and h.CARD_GEN_FLAG(+)='R'
group by C.LOCATION_CODE,M.DESCR,c.area_code
order by C.LOCATION_CODE,substr(c.area_code,4,2),substr(c.area_code,1,3)

----MRS VERIFICATION NEW
