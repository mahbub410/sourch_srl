...
SELECT poi.emp_id, PI.EMP_NO,PRST_OFF_CODE ,PRST_DESIG_CODE,NVL(POI.LAST_PROM_DATE,POI.JOIN_DATE) prst_desig_date, SGD.SUB_GRADE_CODE
...
select SUB_GRADE_CODE,ALLW_CODE, ALLW_AMT, PERC_AMT, PERC_ALLW_CODE,EFFECT_DATE,EXP_DATE
from erp.PMIS_SUB_GRADE_dtl 
where SUB_GRADE_CODE in (
SELECT SUB_GRADE_CODE FROM ERP.PAYROLL_SUB_GR_DESIG
where DESIG_CODE in (select PRST_DESIG_CODE from ERP.HRM_PERSONAL_OFFIC_INFO 
where emp_id='1556'
and PRST_OFF_CODE='000031' )
)
