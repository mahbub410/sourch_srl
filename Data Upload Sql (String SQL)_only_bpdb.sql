...
SELECT * FROM  EPAY_UTILITY_BILL_SUP1;
...
FROM BC_CUSTOMERS@BILLING_Com
WHERE (CUSTOMER_NUM,LOCATION_CODE) IN (SELECT SUBSTR(ACCOUNT_NUMBER,1,7),LOCATION_CODE FROM EPAY_UTILITY_BILL_SUP1)
...
AND LOCATION_CODE='''||LOC||'''
AND INVOICE_NUM IN (SELECT SUBSTR(BILL_NUMBER,1,LENGTH(BILL_NUMBER)-1) FROM EPAY_UTILITY_BILL_SUP1 
                                  WHERE LOCATION_CODE='''||LOC||''');'||CHR(10)||CHR(10)||'COMMIT;'||CHR(10) "QUERY"
FROM (SELECT DISTINCT LOCATION_CODE LOC,:P_BILL_MONTH BILL_MONTH,'@'||'BILLING_TANG' DB_LINK  FROM EPAY_UTILITY_BILL_SUP1)
ORDER BY LOC
